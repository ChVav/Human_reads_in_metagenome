...
for file in "$DIR_IN"/*_1.fastq.gz; do
    ...
    # Run fastp
    singularity exec ./cleanreads.sif fastp -i "$file" -I "$second_file" -o "$first_file_out" -O "$second_file_out" --failed_out "$failed_file" --trim_poly_g --detect_adapter_for_pe --adapter_fasta ./adapters.fa --cut_front 1 --cut_tail 1 -R "$prefix"
    ...
    # Run BBMap
    singularity exec ./cleanreads.sif bbmap.sh t=30 minid=0.95 maxindel=3 bwr=0.16 bw=12 quickmatch fast minhits=2 qtrim=rl trimq=10 untrim in1="$first_file_out" in2="$second_file_out" outu1="$first_final_out" outu2="$second_final_out" outm1="$first_human_out" outm2="$second_human_out"
